...
const NftCreate: NextPage = () => {
  ...
  const [pdfPreviewVisible, setPdfPreviewVisible] = useState(false); // State to manage visibility of the PDF preview

  const handlePdfUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files || e.target.files.length === 0) {
      console.error("Select a PDF file");
      return;
    }
  ...
  const handlePreviewClick = () => {
    setPdfPreviewVisible(true); // Show the PDF preview
  }
