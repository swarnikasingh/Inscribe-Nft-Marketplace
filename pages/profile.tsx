...
const Profile: NextPage = () => {
  ...
  const downloadNft = async (fileUrl: string, name: string) => {
    try {
  ...
  const getNftFileType = (fileUrl: string): string => {
                  ...
                  >
                    {(nfts.data as Nft[]).map((nft) => (
                      <li
                        ...
                        >
                          {getNftFileType(nft.meta.image) === 'Image' ? (
                            <img
                              src={nft.meta.image}
                              alt=""
                              className={classNames(
                            ...
                            <div className="flex items-center justify-center h-full">
                              <p className="text-gray-500 text-lg">{getNftFileType(nft.meta.image)}</p>
                            </div>
                          )}
              ...
              <div className="pb-16 space-y-6">
                <div>
                  <div className="block w-full aspect-w-10 aspect-h-7 rounded-lg overflow-hidden">
                    {getNftFileType(activeNft.meta.image) === 'Image' ? (
                      <img src={activeNft.meta.image} alt="" className="object-cover" />
                    ) : (
                      <div className="flex items-center justify-center h-full bg-gray-100">
                        <p className="text-gray-500 text-lg">{getNftFileType(activeNft.meta.image)} File</p>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex">
                  <button
                    type="button"
                    onClick={() => downloadNft(activeNft.meta.image, activeNft.meta.name)}
                    className="flex-1 bg-indigo-600 py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                  >
                    Download {getNftFileType(activeNft.meta.image)}
                  </button>
                  <button
